...
func NewRelaySwitch(sockets []SocketPair) (*RelaySwitch, error) {
	...
	for masterIdx, sockPair := range sockets {
		socketReader := cbor.NewFrameReader(sockPair.Read)
		socketWriter := cbor.NewFrameWriter(sockPair.Write)
...
func (sw *RelaySwitch) Limits() cbor.Limits {
	sw.mu.Lock()
	defer sw.mu.Unlock()
...
func (sw *RelaySwitch) SendToMaster(frame *cbor.Frame) error {
	sw.mu.Lock()
	defer sw.mu.Unlock()
	...
	case cbor.FrameTypeStreamStart, cbor.FrameTypeChunk, cbor.FrameTypeStreamEnd,
		cbor.FrameTypeEnd, cbor.FrameTypeErr:
		...
		isTerminal := frame.FrameType == cbor.FrameTypeEnd || frame.FrameType == cbor.FrameTypeErr
...
func (sw *RelaySwitch) ReadFromMasters() (*cbor.Frame, error) {
	for {
...
func (sw *RelaySwitch) handleMasterFrame(sourceIdx int, frame *cbor.Frame) (*cbor.Frame, error) {
	switch frame.FrameType {
	case cbor.FrameTypeReq:
		// Peer request
		if frame.Cap == nil {
	...
	case cbor.FrameTypeStreamStart, cbor.FrameTypeChunk, cbor.FrameTypeStreamEnd,
		cbor.FrameTypeEnd, cbor.FrameTypeErr, cbor.FrameTypeLog:
		entry, ok := sw.requestRouting[frame.Id.ToString()]
...
func (sw *RelaySwitch) rebuildLimits() {
	...
	if minFrame == int(^uint(0)>>1) {
		minFrame = cbor.DefaultMaxFrame
	}
	if minChunk == int(^uint(0)>>1) {
		minChunk = cbor.DefaultMaxChunk
	}

	sw.negotiatedLimits = cbor.Limits{
		MaxFrame: minFrame,
		MaxChunk: minChunk,
	}
}
