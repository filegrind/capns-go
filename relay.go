...
type RelaySlave struct {
	localReader    *cbor.FrameReader
	localWriter    *cbor.FrameWriter
	resourceState  []byte
...
func NewRelaySlave(localRead io.Reader, localWrite io.Writer) *RelaySlave {
	return &RelaySlave{
		localReader:   cbor.NewFrameReader(localRead),
		localWriter:   cbor.NewFrameWriter(localWrite),
		resourceState: nil,
	}
}
...
func (rs *RelaySlave) Run(socketRead io.Reader, socketWrite io.Writer, initialNotify *RelayNotifyParams) error {
	socketReader := cbor.NewFrameReader(socketRead)
	socketWriter := cbor.NewFrameWriter(socketWrite)
	...
	go func() {
		...
		for {
			...
			} else if frame.FrameType == cbor.FrameTypeRelayNotify {
	...
	go func() {
		...
		for {
			...
			if frame.FrameType == cbor.FrameTypeRelayNotify || frame.FrameType == cbor.FrameTypeRelayState {
...
func SendNotify(socketWriter *cbor.FrameWriter, manifest []byte, limits cbor.Limits) error {
...
func ConnectRelayMaster(socketReader *cbor.FrameReader) (*RelayMaster, error) {
	frame, err := socketReader.ReadFrame()
	if err != nil {
		if err == io.EOF {
...
func (rm *RelayMaster) Limits() cbor.Limits {
	return rm.limits
}

// SendRelayState sends a RelayState frame to the slave with host system resource info.
func SendRelayState(socketWriter *cbor.FrameWriter, resources []byte) error {
	frame := cbor.NewRelayState(resources)
	return socketWriter.WriteFrame(frame)
}
...
// Returns nil on EOF.
func (rm *RelayMaster) ReadFrame(socketReader *cbor.FrameReader) (*cbor.Frame, error) {
	for {
		frame, err := socketReader.ReadFrame()
		if err != nil {
			if err == io.EOF {
		...
		} else if frame.FrameType == cbor.FrameTypeRelayState {
