...
func CollectArgsByMediaUrn(frames <-chan cbor.Frame, mediaUrnPattern string) ([]interface{}, error) {
...
func CollectFirstArg(frames <-chan cbor.Frame) ([]byte, error) {
			...
			if foundFirst && frame.StreamId != nil && *frame.StreamId == firstStreamID {
				...
				for frame := range frames {
					if frame.FrameType == cbor.FrameTypeEnd {
						return fullData, nil
					} else if frame.FrameType == cbor.FrameTypeErr {
						code := frame.ErrorCode()
						message := frame.ErrorMessage()
						return nil, fmt.Errorf("[%s] %s", code, message)
					}
...
func CollectPeerResponse(frames <-chan cbor.Frame) (map[string][]byte, error) {
...
func CollectAllArgs(frames <-chan cbor.Frame) ([]CapArgumentValue, error) {
